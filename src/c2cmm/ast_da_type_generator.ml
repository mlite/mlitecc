...
let description () = Ast_da_type.description
let suffix () = Ast_da_type.suffix
...
let translate: basename:string -> args:string list -> 
  ...
  fun ~basename ~args ~input:in_chan ~output:out_chan ->
    ...
    in
    let ast = Gen_ast_da_type.compile basename ast
    in
    let ast = Norm_ast_da_type.normalize ast
    in
    Marshal.to_channel out_chan (basename, ast) []
	 
let emit_c_code: input:in_channel -> output:Format.formatter 
  -> unit =
  fun ~input:in_chan ~output:fm ->
    let (basename, ast) = Marshal.from_channel in_chan
    in C_file_basic.Ast_da_type.write_c_formatter fm ast
