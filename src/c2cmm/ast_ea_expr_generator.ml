...
let description () = Ast_ea_expr.description
let suffix () = Ast_ea_expr.suffix
...
let translate: basename:string -> args:string list -> 
  ...
  fun ~basename ~args ~input:in_chan ~output:out_chan ->
	...
	basename ast
    in
    let ast = Norm_ast_ea_expr.normalize typ_db ast
    in
    Marshal.to_channel out_chan (basename, ast, typ_db) []

let emit_c_code: input:in_channel -> output:Format.formatter 
  -> unit =
  fun ~input:in_chan ~output:fm ->
    let ast = Marshal.from_channel in_chan
    in C_file_basic.Ast_ea_expr.write_c_formatter fm ast
