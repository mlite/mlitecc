...
let description () = Ast_aa_gram.description
let suffix () = Ast_aa_gram.suffix
...
let translate: basename:string -> args:string list -> 
  ...
  fun ~basename ~args ~input:in_chan ~output:out_chan ->
    let cabs = Marshal.from_channel in_chan
    in
    let ast = C99ize_cabs.c99ize cabs
...
let emit_c_code: input:in_channel -> output:Format.formatter 
  ...
  fun ~input:in_chan ~output:fm ->
    ...
    in C_file_basic.Ast_aa_gram.write_c_formatter fm ast
