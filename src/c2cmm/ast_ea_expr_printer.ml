...
type c_file_unit = Ast_ea_expr.c_translation_unit
...
let pp_print_c_identifier (fm:formatter) (id:c_identifier):unit =
  pp_open_box fm 0;
  Cent_printer.pp_print_ref_ce fm id;
  pp_close_box fm ()
...
(** *********************************************************************************** **)
let rec pp_print_c_const040: formatter -> 
...
and pp_print_rval: formatter -> rval -> unit =
  fun fm expr ->
	...
	| Rcexpr expr -> pp_print_cexpr fm ~need_paren:true expr
    end;
    pp_close_box fm ()
...
and pp_print_cexpr: formatter -> need_paren:bool -> cexpr -> unit =
  fun fm ~need_paren expr ->
    ...
    begin
      if need_paren then pp_print_string fm "(";
      pp_print_cexpr_ fm ~need_paren:true expr; (*.ce_val;*)
      if need_paren then pp_print_string fm ")";
    end;
    pp_close_box fm ()
      
and pp_print_cexpr_: formatter -> need_paren:bool -> cexpr -> unit =
  fun fm ~need_paren expr ->
    pp_open_hvbox fm indent;
    begin
      match expr with
...
and pp_print_rexpr: formatter -> need_paren:bool -> rexpr -> unit =
  fun fm ~need_paren expr ->
    ...
    begin
      (*if need_paren then pp_print_string fm "(";*)
      pp_print_rexpr_ fm ~need_paren:need_paren expr; (*.re_val;*)
      (*if need_paren then pp_print_string fm ")";*)
    end;
    pp_close_box fm ()
      
and pp_print_rexpr_: formatter -> need_paren:bool -> rexpr -> unit =
  fun fm ~need_paren expr ->
    pp_open_hvbox fm indent;
    begin
      match expr with
...
and pp_print_expr: formatter -> need_paren:bool -> expr -> unit =
  fun fm ~need_paren expr ->
	...
	| Rexpr expr ->
	    begin
	      if need_paren then pp_print_string fm "(";
	      pp_print_rexpr fm ~need_paren:(not need_paren) expr;
	      if need_paren then pp_print_string fm ")";
	    end
...
and pp_print_cast_type fm expr =
  Tent_c_printer.pp_print_c_type_name fm expr
    
and pp_print_decl_type fm expr =
  Tent_c_printer.pp_print_c_type_decl fm expr
...
and pp_print_c_declaration: formatter -> c_declaration -> unit = 
  fun fm expr -> 
	...
	| Type_def te -> 
	    begin
	      pp_print_decl_type fm te
	    end
	      
	| Type_decl c_type ->
	    begin
	      pp_print_decl_type fm c_type
	    end
	| Type_only c_type ->
	    begin
...
and pp_print_c_init_expression: formatter -> c_init_expression -> unit =
  fun fm expr ->
    match expr with
      | Static_init e -> 
...
and pp_print_true_cond: formatter -> true_cond -> unit =
  fun fm true_cond ->
    match true_cond with
	  ...
	  pp_print_rval fm v0;
	  pp_print_space fm ();
	  pp_print_binary_predicate fm brel;
	  pp_print_space fm ();
	  pp_print_rval fm v1;
...
and pp_print_c_stmt010: formatter -> c_stmt010 -> unit = 
  fun fm stmt ->
	...
	| COMPUTATION expr -> assert false
	    
	| SEQUENCE (txt_opt, c_stmt010_list) -> 
	    begin
	      let _ = match txt_opt with
		| Some txt -> fprintf fm "/* sequence: %s */" txt; 
		    pp_print_space fm ();
		| None -> ()
	      in
	      Mlite_printer.pp_print_list fm 
