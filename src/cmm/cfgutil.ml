...
let emit proc cfg call_emit rtl_emit sym_emit =
  ...
  let rec seq_emit = function  (* emit sequence of RTLs only *)
    ...
    | DG.Nop -> ()
    | DG.If _ | DG.While _ -> 
	Impossible.impossible "expected linear code sequence" 
  in
  let block b next () =
    let first f () = match f with
      | GR.Entry -> ()
      | GR.Label ((_, l), _, _) -> sym_emit l 
    in
    let middle m () =
      if GR.is_executable m then
        rtl_emit (GR.mid_instr m) 
    in
    ...
    GR.fold_fwd_block first middle last b () 
  in
...
let numbered_layout_nodes g =
  let add_block (f, t) next (i, nodes') =
    let (i, nodes') = (i+1, (i, F f) :: nodes') 
    in
    let is_next (u', _) = 
      match next with
	| Some (u, _) -> Unique.eq u u'
	| None -> false in
    let rec tail t (i, nodes') = 
      match t with
	| GR.Tail (m, t) -> tail t (i+1, (i, M m) :: nodes')
	| GR.Last (GR.Branch (_, lbl)) when is_next lbl -> (i, nodes') 
	| GR.Last l      -> (i+1, (i, L l) :: nodes') 
    in
    tail t (i, nodes') 
  in
  let _, nodes' = G.fold_layout add_block (0, []) g 
  in
  List.rev nodes'
...
let cfg2dot ~compress ~live ~name g =
  ...
  let number_and_rtl (n, node) =
    let instr rtl = spr "N%d: %s" n (Rtlutil.ToString.rtl rtl) 
    in
    match node with
      | F GR.Entry -> "Entry"
...
let cfg2ast instr g ~name =
  let block b next prev' =
    let mid n prev' = instr (GR.mid_instr n) :: prev' 
    in
    let first n prev' = 
      match n with
	...
	| _ -> instr (GR.last_instr n) :: prev' 
    in
    let rec tail t prev' = 
      match t with
	| GR.Last l -> last l prev'
	| GR.Tail (m, t) -> tail t (mid m prev') 
    in
    let (f, t) = b in
    tail t (first f prev') 
  in
  let oldv = Rtlutil.ToAST.verbosity Rtlutil.ToAST.Low 
  in
  let stmts' = G.fold_layout block [] g 
  in
  let _ = Rtlutil.ToAST.verbosity oldv 
  in
  ( None
    , name
      , []
	, List.rev_map (fun s -> Cmm_ast.StmtBody s) stmts'
	  , Srcmap.null
  )
